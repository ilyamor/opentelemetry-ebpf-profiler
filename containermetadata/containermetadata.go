...
type handler struct {
	...
	containerMetadataHandler ContainerMetadata

	// deferredPID prevents busy loops for PIDs where the cgroup extraction fails.
	deferredPID *lru.SyncedLRU[libpf.PID, libpf.Void]

	// k8sWatcher is a shared informer that listens for changes in the state of pods in the
	// Kubernetes cluster.
	cache *sync.Map
}
...
func GetHandler(ctx context.Context, cfg Config) (Handler, error) {
	...
	instance.deferredPID, err = lru.NewSynced[libpf.PID, libpf.Void](deferredLRUSize,
		libpf.PID.Hash32)
	if err != nil {
		return nil, err
	}
	...
	if os.Getenv(kubernetesServiceHost) != "" {
		err = createK8sWatcher(ctx, instance, cfg)
		if err != nil {
...
func createK8sWatcher(ctx context.Context, handler *handler, cfg Config) error {
	...
	cache := K8sWatcher(nodeName, cfg)
	go func(cache *sync.Map) {
		for {
			cache.Range(func(key, value interface{}) bool {
				load, _ := cache.Load(key)
				loadPod := load.(*K8sPod)
	...
	handler.kubeClientSet, err = kubernetes.NewForConfig(config)
	if err != nil {
		return fmt.Errorf("failed to create Kubernetes client: %v", err)
	}

	_, ok := handler.kubeClientSet.(*kubernetes.Clientset)
	if !ok {
		return fmt.Errorf("failed to create Kubernetes client: %v", err)
	}
	return nil
...
func (h *handler) GetContainerMetadata(pid libpf.PID) (ContainerMetadata, error) {
	...
	if err != nil {
		fmt.Println("container pid not found ", err)
	...
	if data, ok := h.cache.Load(pidContainerID); ok {
		k8sevent, ok := data.(*K8sPod)
		if !ok {
			return ContainerMetadata{}, fmt.Errorf("failed to cast to K8sEntity")

		}
		containerMetadata := ContainerMetadata{pidContainerID, k8sevent.Name, k8sevent.Containers[0].Name, k8sevent.Owner.Name}
		return containerMetadata, nil
	}
...
func (h *handler) getKubernetesPodMetadata(pidContainerID string) (
	...
	for j := range pods.Items {
		...
		serviceName := getServiceName(&pods.Items[j])
